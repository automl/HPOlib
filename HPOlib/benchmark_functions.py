...
def save_lda_on_grid(params, **kwargs):
    ...
    if type(kappa) == np.ndarray:
        ...
        tau = tau[0]
        s = s[0]
    return lda_on_grid(kappa, tau, s)


def lda_on_grid(kappa, tau, s):
    ...
    for kappa_value in kappa_values:
        ...
        for Tau_value in tau_values:
            ...
            for S_value in s_values:
                config_tree[kappa_value][Tau_value][S_value] = np.inf

    # Populate the tree
    for config in configurations:
        config_tree[config[0]][config[1]][config[2]] = config[3]

    print kappa, tau, s
    return config_tree[kappa_values[int(float(kappa))]][tau_values[int(float(tau))]][s_values[int(float(s))]]